  ...
  (0..5).step(0.5).each do |noise_scale|#(0..5).each do |noise_level| 
    ...
    [0, 1].each do |open_loop|
      [10].each do |collision_clearance_coeff|
        %w[needle_steering_11002].each do |pg_name|
          [1, 2].each do |method|
            [[1, 0]].each do |separate_planning_first, simultaneous_planning|
              ...
              Record.create! exp_options.merge command: command, result: result, pg_name: pg_name, problem: :needle_steering, version: 11002
            end
          end
        end
      end
