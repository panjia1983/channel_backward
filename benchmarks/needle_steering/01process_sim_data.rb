...
selectorr = Record.where(version: 30)

puts "#{selectorr.count} records in total"

channels = selectorr.where(problem: :channel_planning)
needles = selectorr.where(problem: :needle_steering)
...
channels.where(converged: nil).each do |record|
  record.converged = record.result.scan(/channel planning converged/).count > 0
  record.save!
end
