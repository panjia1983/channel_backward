  ...
  %w[needle_steering_separate_penalty needle_steering_same_penalty].each do |pg_name|
    [1, 2].each do |method|
      [[1, 0], [1, 1], [0, 1]].each do |separate_planning_first, simultaneous_planning|
        ...
        Record.create! exp_options.merge command: command, result: result, pg_name: pg_name, problem: :needle_steering, version: 32, description: "5 needle simulation test"
      end
    end
  end
end
