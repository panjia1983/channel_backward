...
selectorr = Record.where(version: 37)
...
[1, 10].each do |collision_clearance_coeff|
  ["needle_steering_same_penalty"].each do |pg_name|
    [1, 2].each do |method|
      [[0, 1], [1, 0]].each do |separate_planning_first, simultaneous_planning|
        ...
        success_cnt = needles.where(range).where(collision_status: [0]).where(:distance_to_goal.lt => 0.50).count
