...
[1, 2].each do |method|
  [[1, 0]].each do |separate_planning_first, simultaneous_planning|
    [1, 10].each do |collision_clearance_coeff|
      converged_points = Record.where(version: 10502,
                                      converged: true,
...
all_converged_pts = each_converged.inject(:&)
    ...
    %w[needle_steering_small_noise].each do |pg_name|
      [1, 2].each do |method|
        [[1, 0], [0, 1]].each do |separate_planning_first, simultaneous_planning|
          ...
          Record.create! exp_options.merge command: command, result: result, pg_name: pg_name, problem: :needle_steering, version: 10803
        end
      end
    end
  end
