...
selectorr = Record.where(version: 32)
...
selectorr.where(collision_status: nil).each do |record|
  record.collision_status = record.result.scan(/collision status: (.*)$/).count > 0
  record.save!
end

selectorr.where(distance_to_goals: nil).each do |record|
  record.collision_status = record.result.scan(/distance to goals: (.*)$/).count > 0
  record.save!
end
...
#  csv << %w[pg_name method separate_planning_first simultaneous_planning average_collision_free_count average_collision_free_distance_to_goal average_run_time]
  ...
  needles.pluck(:pg_name).uniq.each do |pg_name|
    [1, 2].each do |method|
      [[1, 0], [1, 1], [0, 1]].each do |separate_planning_first, simultaneous_planning|
        range = {
          pg_name: pg_name,
          method: method,
        ...
        collision_free_cnt = needles.where(range).pluck(&:collision_status).map(&:size).select{|x| x==5}.count#where(converged:true).count
        ...
        puts "avg run time: #{total_run_time*1.0/cnt}"
        puts "avg collision free cnt: #{collision_free_cnt*1.0/cnt}"
